...
class VisitHandler(ActionHandler):
    async def handle(self, action_with_think: ActionWithThinkB) -> None:
        ...
        unvisited_urls = [
            url for url in normalized_urls if url not in self.state.visited_urls
        ]

        top_k_unvisited_urls = choose_k(
            unvisited_urls, self.state.config.max_urls_per_step
        )
        ...
        if len(top_k_unvisited_urls) > 0:
            ...
            url_results = await asyncio.gather(
                *[self._handle_url(url) for url in top_k_unvisited_urls]
            )

            # Check if we've found useful information
            success = any(
                r and r.get("result", {}).get("content", "").strip()
                for r in url_results
            )

            await self._send_event(
                Event.VISIT.value, {"urls": url_ids, "results": url_results}
            )
            if success:
                ...
                for r in url_results:
                    if r and r.get("url", "") in self.state.all_urls:
                        diary_entries.append(
                            self.state.get_display_url(r.get("url", ""))
                        )
                ...
                self._add_to_diary(
                    VISIT_FAIL_PROMPT.format(
                        step=self.state.step,
                    )
                )
        else:
            self._add_to_diary(
                VISIT_DUPLICATE_PROMPT.format(
                    step=self.state.step,
                )
            )
